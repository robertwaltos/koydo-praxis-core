...
import { EXAM_CONFIG } from "@/lib/praxis-core/config";

describe("Praxis Core Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM163");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("praxis-core");
  });
