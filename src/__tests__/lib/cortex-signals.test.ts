...
import { emitPrismSignal } from "@/lib/praxis-core/cortex-signals";

describe("PRISM Signals", () => {
  it("dispatches custom event on window", () => {
    const spy = vi.fn();
    window.addEventListener("koydo:prism-signal", spy);
    emitPrismSignal({ eventType: "question_answered", activityType: "praxis-core" });
  ...
  it("includes correct event detail", () => {
    ...
    emitPrismSignal({ eventType: "exam_started", value: 1, activityType: "praxis-core" });
    expect(detail).toEqual({ eventType: "exam_started", value: 1, activityType: "praxis-core" });
